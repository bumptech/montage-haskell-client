...
testReferenceGet pool = do
    ...
    case valuesFound of
        Just vs -> case length vs == 2 of
            ...
            False -> error $ "\nLooking for 2, found: " ++ (show $ length vs)
        Nothing -> error $ "\nReference get found no values"

    valuesFoundTwice <- montageGetBy pool "u-info" (toKey 1) ["u-event", "u-name"]
    if valuesFound == valuesFoundTwice then hPutStrLn stdout "\nConfirmed reference get" else error "\nCould not confirm reference get"
