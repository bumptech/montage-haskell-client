...
module Network.Riak.MontageClient.Proto.MontageClient.MontageCommandResponse (MontageCommandResponse(..)) where
import Prelude ((+), (/))
...
data MontageCommandResponse = MontageCommandResponse{status :: !P'.Utf8, value :: !(P'.Maybe P'.ByteString)}
                            deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data)
 
instance P'.Mergeable MontageCommandResponse where
  mergeAppend (MontageCommandResponse x'1 x'2) (MontageCommandResponse y'1 y'2)
   = MontageCommandResponse (P'.mergeAppend x'1 y'1) (P'.mergeAppend x'2 y'2)
 
instance P'.Default MontageCommandResponse where
  defaultValue = MontageCommandResponse P'.defaultValue P'.defaultValue
 
instance P'.Wire MontageCommandResponse where
  wireSize ft' self'@(MontageCommandResponse x'1 x'2)
   = case ft' of
       10 -> calc'Size
  ...
  wirePut ft' self'@(MontageCommandResponse x'1 x'2)
   = case ft' of
       10 -> put'Fields
       11 -> do
